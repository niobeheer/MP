...
function measurement(consent,id){
 let current=consent;const handlers={},scripts=[],calls=[];
 const win={CK_MEASUREMENT_CONFIG:{ga4Id:id},gtag:(...args)=>calls.push(args),addEventListener(){}};
...
const report={version:136,html:files.length,profiles,tracked_pages:tracked,parseable_schema_blocks:schema,distinct_profile_summaries:summaries.size,duplicate_title_groups:duplicates.length,duplicates,metadata_warnings:warnings,failures,filter_logic:'PASS',filter_css:'PASS (static assertion, not rendered)',measurement_contract:'PASS (isolated fixture, not GA receipt)',live_feed_tests:'NOT COMPLETED: external requests did not finish',visual_browser_test:'NOT RUN: no local browser executable',analytics_activation:'BLOCKED: verified GA4 ID required',monthly_email:'NOT SCHEDULED: data connection and recipient required'};
fs.writeFileSync(path.join(root,'QA-V136-final.json'),JSON.stringify(report,null,2));console.log(JSON.stringify({...report,duplicates:duplicates.slice(0,4),metadata_warnings:warnings.slice(0,4)},null,2));if(failures.length)process.exit(1);
